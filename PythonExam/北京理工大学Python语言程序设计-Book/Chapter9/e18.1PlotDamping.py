...
def XY_Axis(x_start, x_end, y_start, y_end):
    ...
    plt.xticks([np.pi/3, 2*np.pi/3, 1*np.pi, 4*np.pi/3, \
                5*np.pi/3],['$\pi/3$','$2\pi/3$','$\pi$','$4\pi/3$',\
                            '$5\pi/3$'])
...
Shadow(0.8, 3)
plt.legend()
plt.savefig('e18.1PlotDamping_sample.jpg')
plt.show()
'''
...
def Shadow(a, b):
    ix = (x>a) & (x<b)
    plt.fill_between(x,y,0,where=ix,facecolor='orange', alpha=0.25)
